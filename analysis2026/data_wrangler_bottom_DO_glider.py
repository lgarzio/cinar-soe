...
https://gliders.ioos.us/erddap/index.html using download_DO_gliders.py and export as NetCDF.
"""
...
def main(fdir, lon_bounds, lat_bounds, yr):
    ...
    oxygen_varnames = ['dissolved_oxygen', 'oxy4_oxygen', 'oxygen', 'oxy3835_wphase_oxygen']
    ...
    for gf in sorted(glob.glob(os.path.join(fdir, '*.nc'))):
        ...
        # convert DO to mg/L
        if ds[oxyvar].units in['umol kg-1', 'micromol kg-1', 'micromoles L-1', 'umol kg-3']:  # umol kg-3 is a typo it should be umol kg-1
        ...
        for pid in ds.profile.values:
            ...
            if pd.to_datetime(pds.time.values).year != yr:
                continue
...
if __name__ == '__main__':
    ...
    lon_extent = [-78, -65]  # longitude boundaries for grabbing data in the MAB
    lat_extent = [35, 45]  # latitude boundaries for grabbing data in the MAB
    year = 2025
