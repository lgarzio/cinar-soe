...
def main(lon_bounds, lat_bounds, codap_file, extra_files, underway_files, savedir):
    ...
    with open(os.path.join(home_dir, extra_files), "r") as file:
        extra_files_list = yaml.safe_load(file)
    extra_files = extra_files_list.split(' ')
    extra_files = [os.path.join(home_dir, x) for x in extra_files]

    with open(os.path.join(home_dir, underway_files), "r") as file:
        underway_files_list = yaml.safe_load(file)
    underway_files = underway_files_list.split(' ')
    underway_files = [os.path.join(home_dir, x) for x in underway_files]
...
#######################################################################################################################
    # get CODAP data
    df = pd.read_csv(os.path.join(home_dir, codap_file))
...
#######################################################################################################################
    ...
    for ef in extra_files:
        ...
        try:
            coord_vars = ['Longitude_Dec_Deg', 'Latitude_Dec_Deg']
            test = df[coord_vars[0]]
        except KeyError:
            coord_vars = ['Longitude', 'Latitude']
            
        # calculate pressure from depth and latitude
        df['press_dbar'] = p_from_z(-df[depthvar], df[coord_vars[1]])
        ...
        # combine temperature and salinity columns if they are split into two columns
        try:
            df['TEMP'] = df['CTDTEMP_ITS90'].combine_first(df['TSGTEMP_deg_C'])
            df['SAL'] = df['CTDSAL_PSS78'].combine_first(df['TSGSAL_PSS78'])
        except KeyError:
            df['TEMP'] = df['CTDTEMP_ITS90']
            df['SAL'] = df['CTDSAL_PSS78']
        ...
        kwargs = dict(salinity=df.SAL.values,
                      temperature=df['report_temp'].values,
                      temperature_out=df.TEMP.values,
                      pressure=0,
                      pressure_out=df.press_dbar.values,
                      opt_pH_scale=1,
                      opt_k_carbonic=4,
                      opt_k_bisulfate=1,
                      opt_total_borate=1,
        ...
        # save flow-through data for later use
        df_ft = df[df.Observation_Type == 'Flow-through']
        df_ft = df_ft.dropna(subset=['pH_insitu'])

        df = df[df.Observation_Type != 'Flow-through']
        
        # for each profile, find the shallowest sample
        profile_num = np.unique(df.profile_id)
        for profile in profile_num:
            df_profile = df[df.profile_id == profile]
            profile_coords = [df_profile[coord_vars[0]].values[0], df_profile[coord_vars[1]].values[0]]
        ...
        # add the flow thru data
        if len(df_ft) > 0:

            # convert time to timestamp
            tm = df_ft['time'].map(lambda t: pd.to_datetime(t).timestamp())
            ...
            data['data_vars']['lon']['data'] = np.append(data['data_vars']['lon']['data'], df_ft[coord_vars[0]].values)

            data['data_vars']['pH_surface']['data'] = np.append(data['data_vars']['pH_surface']['data'], df_ft['pH_insitu'].values)
            data['data_vars']['omega_surface']['data'] = np.append(data['data_vars']['omega_surface']['data'], df_ft['aragonite'].values)
            data['data_vars']['temperature_surface']['data'] = np.append(data['data_vars']['temperature_surface']['data'], df_ft['TEMP'].values)
...
########################################################################################################################
    ...
    save_file = os.path.join(savedir, f'vessel_based_OA_data_{start_yr}_{end_yr}.nc')
...
if __name__ == '__main__':
    ...
    codap = 'rucool/Saba/OA_cruise_data/CODAPv2021/CODAP-NA_v2021-filtered-for-analysis.csv'
    other = 'rucool/Saba/OA_cruise_data/soe_other_filepaths.yml'
    other_underway = 'rucool/Saba/OA_cruise_data/soe_other_underway_filepaths.yml'
    save_directory = 'rucool/Saba/NOAA_SOE/data/output_nc'
