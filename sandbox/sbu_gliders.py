...
f = '/Users/garzio/Documents/rucool/Saba/gliderdata/charlie/sbu01_04_final_May2021.nc'
#f = '/Users/garzio/Documents/rucool/Saba/gliderdata/charlie/sbu01_05_final_July2021.nc'
calfile = '/Users/garzio/Documents/repo/lgarzio/phglider/calibration/sbe10528_20200821.txt'  # both deployments have the same cal file
with open(calfile) as json_file:
    cc = json.load(json_file)

ds = xr.open_dataset(f)
#profiletime = nc.num2date(ds.profiletime.values, units='seconds since 1970-01-01', calendar='standard')
df = ds.to_dataframe()

# calculate pH and add to dataframe
df['f_p'] = np.polyval([cc['f6'], cc['f5'], cc['f4'], cc['f3'], cc['f2'], cc['f1'], 0], df.pressure)
phfree, phtot = phcalc(df.pH, df.pressure, df.temperature, df.salinity, cc['k0'], cc['k2'], df.f_p)
df['ph_total'] = phtot
