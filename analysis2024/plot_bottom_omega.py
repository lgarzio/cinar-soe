...
def main(cruise_file, glider_file, ab, savedir, thresh):
    ...
    savedir = os.path.join(savedir, 'species_omega_sensitivity')
    for key, value in thresh.items():
        ...
        # year_idx = np.where(pd.to_datetime(time_full_region).year < 2023)[0]
        # lon_full_region = lon_full_region[year_idx]
        # lat_full_region = lat_full_region[year_idx]
        # data_full_region = data_full_region[year_idx]
        # depth_full_region = depth_full_region[year_idx]
        # time_full_region = time_full_region[year_idx]
        ...
        # plot 2023 in a different color
        idx2023 = np.where(pd.to_datetime(time_full_region[mask]).year == 2023)[0]
        sct = ax.scatter(lon_full_region[mask][idx2023], lat_full_region[mask][idx2023], c='magenta',
                         marker='o', s=20, transform=ccrs.PlateCarree(), zorder=10)
        ...
        min_year1 = np.nanmin(np.unique(pd.to_datetime(time_full_region).year))
        max_year1 = np.nanmax(np.unique(pd.to_datetime(time_full_region).year))
        sfile = os.path.join(savedir, f'bottom_omega_map-{key}-{min_year1}-{max_year1}.png')
        plt.savefig(sfile, dpi=200)
        plt.close()
